...
hook.Add("SlashCo:DrawHUD", "BaseSlasherHUD", function()
	...
	if pacified then
		draw.SimpleText(SlashCo.Language("demon_pacified"),
	...
	for _, v in ipairs(ents.FindByClass("sc_activebeacon")) do
		if v:GetArmingBeacon() then
			...
			draw.SimpleText(SlashCo.Language("beacon_call"), "ItemFontTip", ScrW() / 2,
					ScrH() / 1.5, Color(255, 0, 0, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
			draw.SimpleText(SlashCo.Language("beacon_destroy"), "ItemFontTip", ScrW() / 2, ScrH() / 1.45, Color(255, 0, 0, 255),
	...
	if g < 1 then
		surface.DrawTexturedRect((cx - (ScrW() / 4) + (gp * (cx))) - (ScrW() / 60) - (pulse / 2),
				(cy / 7.5) - (pulse / 2), ScrW() / 30 + (pulse), ScrW() / 30 + (pulse))
	else
		surface.DrawTexturedRect((cx - (ScrW() / 4) + (gp * (cx))) - (ScrW() / 60) - (pulse / 2) - 25,
				(cy / 7.5) - (pulse / 2) - 25, ScrW() / 30 + (pulse) + 50, ScrW() / 30 + (pulse) + 50)
	end
	...
	if CanKill then
		...
		draw.SimpleText("M1 - Kill Survivor", "ItemFontTip", mainiconposx + (cx / 8), mainiconposy - (cy / 4),
				Color(255, 0, 0, 255), TEXT_ALIGN_BOTTOM, TEXT_ALIGN_LEFT)
	else
	...
	if CanChase then
		...
		if not inchase then
			draw.SimpleText("M2 - Start Chasing", "ItemFontTip", mainiconposx + (cx / 8), mainiconposy - (cy / 2),
					Color(255, 0, 0, 255), TEXT_ALIGN_BOTTOM, TEXT_ALIGN_LEFT)
		else
		...
		surface.DrawTexturedRect(mainiconposx, mainiconposy - (cy / 2), ScrW() / 16, ScrW() / 16)
		draw.SimpleText("-Unavailable-", "ItemFontTip", mainiconposx + (cx / 8), mainiconposy - (cy / 2),
				Color(100, 0, 0, 255), TEXT_ALIGN_BOTTOM, TEXT_ALIGN_LEFT)
	end
